...
const DEFAULT_MONDAY_TOKEN = "eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjYwNjIzNTUxMSwiYWFpIjoxMSwidWlkIjo3Mzg2OTI3NiwiaWFkIjoiMjAyNi0wMS0xMlQxMTo1OToxMC45MjVaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6ODQ1ODk5OSwicmduIjoidXNlMSJ9.10toILN0Q84wOPf1s3lZH-NvLubh053ZxdqA5wqaqEQ";
...
/* ===== CUSTOMS CALCULATION SYSTEM ===== */

// Monday.com Boards for Customs
const BOARD_EXCHANGE_RATES = 7406444729;  // أسعار الصرف
const BOARD_COMMODITIES = 7406443393;     // أنواع البضائع (same as BOARD_PRODUCT_PRICING)

// Exchange Rates Column IDs
const COLUMN_IDS_EXCHANGE_RATES = {
  southMarketFx: "numeric_mkyhrswn",    // سعر السوق مقابل الدولار – جنوب
  customsFxNorth: "numeric_mkyhqzar",   // سعر الصرف الجمركي – شمال
  customsFxSouth: "numeric_mkyhhzkf",   // سعر الصرف الجمركي – جنوب
  avgFxRate: "numeric_mkyhqmtz"         // سعر السوق – شمال
};

// Commodity Column IDs (for customs)
const COMMODITY_COLUMN_IDS = {
  commodityType: "text_mkwdfwyy",           // نوع البضاعة
  tariffSouth: "numeric_mkwbvqc1",          // التعرفة الجمركية - جنوب
  tariffNorth: "numeric_mkwyy2fn",          // التعرفة الجمركية - شمال
  salesTax: "numeric_mkwxk61m",             // ضريبة المبيعات
  vat: "numeric_mkwyn5b8",                  // ضريبة القيمة المضافة
  profitTax: "numeric_mkwyw5dx"             // ضريبة الأرباح
};

// Yemen Provinces Classification
const NORTHERN_PROVINCES = [
  "صنعاء", "أمانة العاصمة", "صعدة", "عمران", "حجة", "المحويت",
  "ريمة", "ذمار", "إب", "تعز", "تعز الحوبان", "الحديدة", "مأرب", "الجوف", "البيضاء"
];

const SOUTHERN_PROVINCES = [
  "عدن", "لحج", "أبين", "الضالع", "شبوة", "حضرموت",
  "المهرة", "سقطرى", "تعز المدينة"
];

// Global storage for customs data
window.__WEPS_EXCHANGE_RATES = null;
window.__WEPS_COMMODITIES_DATA = [];
...
function calculateAllServices({
  ...
  cargoValue,
  clothesBaseRates,
  vasCost = { totalCost: 0, services: [], breakdown: {} },
  options = {
    useRouteMultiplier: true,
    includeProfit: true,
  ...
  allServices.forEach(serviceCode => {
    ...
    if (cargoValue >= 0) {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/8d2fc7b0-c289-4711-8e2f-96141aa9ec6c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'script.js:1285', message: 'calculateAllServices: customsResult', data: { serviceCode: serviceCode, cargoValue: cargoValue, hasCustomsResult: !!customsResult, customsSuccess: customsResult?.success, customsTotalUSD: customsResult?.calculation?.total_customs_usd, customsFull: JSON.stringify(customsResult) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'C' }) }).catch(() => { });
      // #endregion
    ...
    // Add VAS cost to total price
    const vasCostAmount = vasCost?.totalCost || 0;
    totalPrice += vasCostAmount;
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/8d2fc7b0-c289-4711-8e2f-96141aa9ec6c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'script.js:1310', message: 'calculateAllServices: result stored', data: { serviceCode: serviceCode, hasCustoms: !!results[serviceCode].customs, customsSuccess: results[serviceCode].customs?.success, customsTotalUSD: results[serviceCode].customs?.calculation?.total_customs_usd }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'E' }) }).catch(() => { });
    // #endregion
  });
...
const WAREHOUSES = {
  shopAndShip: [
    ...
    // United States
    { name: 'WEPS Warehouse - New York', country: 'US', lat: 40.7128, lng: -74.0060, address: '1234 Commerce Street, Brooklyn, NY 11201, United States', dxbNumber: 'DXB 1702964', phone: '+1 718 555 1234' },
    { name: 'WEPS Warehouse - Los Angeles', country: 'US', lat: 34.0522, lng: -118.2437, address: '5678 Industrial Boulevard, Los Angeles, CA 90001, United States', dxbNumber: 'DXB 1702964', phone: '+1 213 555 5678' },
    ...
    // Brazil
    { name: 'WEPS Warehouse - Sao Paulo', country: 'BR', lat: -23.5505, lng: -46.6333, address: 'Av. Paulista, 1000 - Bela Vista, São Paulo - SP, 01310-100, Brazil', dxbNumber: 'DXB 1702964', phone: '+55 11 5555 1234' },
    { name: 'WEPS Warehouse - Rio de Janeiro', country: 'BR', lat: -22.9068, lng: -43.1729, address: 'Rua do Ouvidor, 50 - Centro, Rio de Janeiro - RJ, 20040-030, Brazil', dxbNumber: 'DXB 1702964', phone: '+55 21 5555 5678' },
    // Belgium
    { name: 'WEPS Warehouse - Brussels', country: 'BE', lat: 50.8503, lng: 4.3517, address: 'Rue de la Loi 200, 1040 Brussels, Belgium', dxbNumber: 'DXB 1702964', phone: '+32 2 555 1234' },
    { name: 'WEPS Warehouse - Antwerp', country: 'BE', lat: 51.2194, lng: 4.4025, address: 'Havenhuis, Zaha Hadidplein 1, 2030 Antwerp, Belgium', dxbNumber: 'DXB 1702964', phone: '+32 3 555 5678' },
    // Denmark
    { name: 'WEPS Warehouse - Copenhagen', country: 'DK', lat: 55.6761, lng: 12.5683, address: 'Vesterbrogade 3, 1620 Copenhagen V, Denmark', dxbNumber: 'DXB 1702964', phone: '+45 33 55 1234' },
    { name: 'WEPS Warehouse - Aarhus', country: 'DK', lat: 56.1629, lng: 10.2039, address: 'Banegårdspladsen 1, 8000 Aarhus, Denmark', dxbNumber: 'DXB 1702964', phone: '+45 86 55 5678' },
    // Finland
    { name: 'WEPS Warehouse - Helsinki', country: 'FI', lat: 60.1699, lng: 24.9384, address: 'Mannerheimintie 1, 00100 Helsinki, Finland', dxbNumber: 'DXB 1702964', phone: '+358 9 555 1234' },
    { name: 'WEPS Warehouse - Tampere', country: 'FI', lat: 61.4978, lng: 23.7610, address: 'Hämeenkatu 1, 33100 Tampere, Finland', dxbNumber: 'DXB 1702964', phone: '+358 3 555 5678' },
    // Norway
    { name: 'WEPS Warehouse - Oslo', country: 'NO', lat: 59.9139, lng: 10.7522, address: 'Karl Johans gate 1, 0162 Oslo, Norway', dxbNumber: 'DXB 1702964', phone: '+47 22 55 1234' },
    { name: 'WEPS Warehouse - Bergen', country: 'NO', lat: 60.3913, lng: 5.3221, address: 'Bryggen 1, 5003 Bergen, Norway', dxbNumber: 'DXB 1702964', phone: '+47 55 55 5678' },
    // Austria
    { name: 'WEPS Warehouse - Vienna', country: 'AT', lat: 48.2082, lng: 16.3738, address: 'Stephansplatz 1, 1010 Vienna, Austria', dxbNumber: 'DXB 1702964', phone: '+43 1 555 1234' },
    { name: 'WEPS Warehouse - Graz', country: 'AT', lat: 47.0707, lng: 15.4395, address: 'Hauptplatz 1, 8010 Graz, Austria', dxbNumber: 'DXB 1702964', phone: '+43 316 555 5678' },
    // Portugal
    { name: 'WEPS Warehouse - Lisbon', country: 'PT', lat: 38.7223, lng: -9.1393, address: 'Praça do Comércio, 1100-148 Lisbon, Portugal', dxbNumber: 'DXB 1702964', phone: '+351 21 555 1234' },
    { name: 'WEPS Warehouse - Porto', country: 'PT', lat: 41.1579, lng: -8.6291, address: 'Praça da Liberdade, 4000-322 Porto, Portugal', dxbNumber: 'DXB 1702964', phone: '+351 22 555 5678' },
    // Romania
    { name: 'WEPS Warehouse - Bucharest', country: 'RO', lat: 44.4268, lng: 26.1025, address: 'Calea Victoriei 1, 010061 Bucharest, Romania', dxbNumber: 'DXB 1702964', phone: '+40 21 555 1234' },
    { name: 'WEPS Warehouse - Cluj-Napoca', country: 'RO', lat: 46.7712, lng: 23.6236, address: 'Strada Memorandumului 28, 400114 Cluj-Napoca, Romania', dxbNumber: 'DXB 1702964', phone: '+40 264 555 5678' }
  ],
  china: [
...
function calculateVASCost() {
  const selectedKeys = getSelectedVASKeys();
  ...
  selectedKeys.forEach(key => {
    ...
    if (key === 'photo') {
      cost = option.costUSD || 5;
    } else if (key === 'inspection') {
      cost = option.costUSD || 30;
    } else if (key === 'purchase') {
      // حساب 10% من قيمة المنتجات المضافة في purchaseProductsList
      const purchaseProductsList = document.getElementById('purchaseProductsList');
      let purchaseProductsTotal = 0;

      if (purchaseProductsList) {
        // البحث عن جميع حقول قيمة المنتجات في القائمة
        // محاولة البحث بعدة طرق مختلفة
        const productValueInputs = purchaseProductsList.querySelectorAll('input[type="number"][data-product-value], input[type="number"].product-value, input[type="number"][name*="value"], input[type="number"][name*="price"]');
        productValueInputs.forEach(input => {
          const value = parseFloat(input.value) || 0;
          purchaseProductsTotal += value;
        });

        // إذا لم نجد حقول محددة، نبحث عن أي input number في القائمة
        if (productValueInputs.length === 0) {
          const allNumberInputs = purchaseProductsList.querySelectorAll('input[type="number"]');
          allNumberInputs.forEach(input => {
            // تخطي حقول الكمية والأبعاد، نبحث عن القيمة فقط
            const name = (input.name || '').toLowerCase();
            const id = (input.id || '').toLowerCase();
            if (name.includes('value') || name.includes('price') || name.includes('cost') ||
              id.includes('value') || id.includes('price') || id.includes('cost')) {
              const value = parseFloat(input.value) || 0;
              purchaseProductsTotal += value;
            }
          });
      ...
      cost = purchaseProductsTotal * (option.costPercentage || 0.10);
    }
...
function initStepAccordions() {
  ...
  // Initialize: All blocks collapsed by default
  ...
  // Open first block (data-step="1") by default
  const firstBlock = document.querySelector('.step-block[data-step="1"]');
  if (firstBlock) {
    firstBlock.classList.remove('collapsed');
    const headerBtn = firstBlock.querySelector('.step-header-btn');
    if (headerBtn) {
  ...
  blocks.forEach(block => {
    ...
    // Only click functionality - no hover
...
function initHelpPopupAutoClose() {
  const helpToggle = document.getElementById('helpToggle');
  const helpPopup = document.getElementById('helpPopup');
  
  if (!helpToggle || !helpPopup) return;
  
  let closeTimer = null;
  
  // دالة مساعدة للتحقق من إمكانية الإغلاق
  const canClose = () => {
    const isMouseOverPopup = helpPopup.matches(':hover');
    const isMouseOverToggle = helpToggle.matches(':hover');
    return !isMouseOverPopup && !isMouseOverToggle;
  };
  
  // دالة مساعدة للإغلاق
  const closePopup = () => {
    if (helpPopup.classList.contains('open') && canClose()) {
      helpPopup.classList.remove('open');
      helpPopup.setAttribute('aria-hidden', 'true');
    }
  };
  
  // إلغاء timer عند دخول الماوس للنافذة
  ...
  // بدء timer للإغلاق عند خروج الماوس من النافذة
  ...
  // إلغاء timer عند دخول الماوس للزر
  ...
  // بدء timer للإغلاق عند خروج الماوس من الزر (إذا كانت النافذة مفتوحة)
  helpToggle.addEventListener('mouseleave', () => {
    // التحقق من أن النافذة مفتوحة أولاً
...
async function getProductsFromMonday() {
      ...
      items_page(limit:200){
        items {
          ...
          subitems {
            id
            name
            column_values(ids:["text_mkzcjgpk"]){
              id
              text
            }
          }
  ...
  for (const it of items) {
    ...
    if (p) {
      // Extract categories from subitems
      const categories = it.subitems?.map(sub => {
        const catCol = sub.column_values?.find(cv => cv.id === 'text_mkzcjgpk');
        return catCol?.text?.trim();
      }).filter(Boolean) || [];
...
function initProductCategoryDropdown() {
  ...
  productTypeSelect.addEventListener('change', function() {
    ...
    const products = window.__WEPS_PRODUCTS || [];
    const productData = products.find(p => p.product === selectedProduct);
    ...
    if (productData && productData.categories && productData.categories.length > 0) {
      ...
      productCategorySelect.innerHTML = '<option value="">لا توجد فئات متاحة</option>';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize product category dropdown
  ...
  if (serviceTypeEl && serviceTypeDescEl) {
    const serviceDescriptions = {
      'express_yemen': '⚡ أسرع خيار - توصيل مباشر إلى اليمن في 10-14 أيام (من دبي فقط)',
      'air_eco_uae_oman_truck_yemen': '✈️ توازن مثالي بين السعر والسرعة - 18-23 يوم',
      'air_aden_via_eg_jo': '🛫 خيار ممتاز لعدن والمناطق الجنوبية - 12-18 يوم',
      'sea_kg_via_uae_truck_yemen': '🚢 اقتصادي للأوزان المتوسطة - 55-65 يوم',
      'sea_cbm_via_uae_truck_yemen': '⛴️ الأفضل للشحنات الكبيرة - 35-45 يوم',
      'sea_cbm_direct_aden_mukalla': '🚤 الأرخص للشحنات الكبيرة إلى الجنوب - 55-65 يوم'
    };
...
// Validation for customer name (text only, no numbers)
if (customerNameInputEl) {
  customerNameInputEl.addEventListener('input', () => {
    // Remove any numbers from input
    let value = customerNameInputEl.value.replace(/[0-9]/g, '');
    if (customerNameInputEl.value !== value) {
      customerNameInputEl.value = value;
    }
  });
  customerNameInputEl.addEventListener('blur', () => {
    // Validate: only Arabic/English letters and spaces
    const namePattern = /^[a-zA-Z\u0600-\u06FF\s]+$/;
    if (customerNameInputEl.value && !namePattern.test(customerNameInputEl.value)) {
      customerNameInputEl.setCustomValidity('يجب إدخال نص فقط (بدون أرقام)');
      customerNameInputEl.reportValidity();
    } else {
      customerNameInputEl.setCustomValidity('');
    }
  });
}
...
if (customerPhoneEl) {
  customerPhoneEl.addEventListener('input', () => {
    updatePhoneFlag(customerPhoneEl, customerPhoneFlagEl, parsedDataCache?.customer?.country || 'yemen');
    
    // Remove country code (+967) if present
    let value = customerPhoneEl.value.replace(/\+967\s*/g, ''); // Remove +967
    value = value.replace(/\D/g, ''); // Remove non-digits
    if (value.length > 9) value = value.slice(0, 9); // Limit to 9 digits
    
    if (customerPhoneEl.value !== value) customerPhoneEl.value = value;
  });
  customerPhoneEl.addEventListener('blur', () => {
    // Extract only the 9 digits (remove country code if present)
    let phoneValue = customerPhoneEl.value.replace(/\+967\s*/g, ''); // Remove +967
    phoneValue = phoneValue.replace(/\D/g, ''); // Remove non-digits
    
    // Validate: must be exactly 9 digits
    if (phoneValue && phoneValue.length !== 9) {
      customerPhoneEl.setCustomValidity('يجب إدخال رقم يمني مكون من 9 أرقام فقط (بدون مفتاح الدولة)');
      customerPhoneEl.reportValidity();
    } else {
      customerPhoneEl.setCustomValidity('');
      // Format with country code for display/storage
      if (phoneValue && phoneValue.length === 9) {
        setPhoneField(customerPhoneEl, customerPhoneFlagEl, phoneValue, 'yemen');
      }
    }
  });
}
...
function addProductRow() {
  if (!purchaseProductsList) return;
  
  const productIndex = purchaseProductsList.children.length;
  const productRow = document.createElement('div');
  productRow.className = 'purchase-product-item';
  productRow.style.cssText = 'margin-bottom:12px;padding:12px;background:#fff;border-radius:8px;border:1px solid rgba(11,18,32,0.1);position:relative';
  
  productRow.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      <label style="font-weight:700;color:var(--weps-navy)">منتج #${productIndex + 1}</label>
      <button type="button" class="remove-product-btn" style="background:#ef4444;color:#fff;border:none;border-radius:6px;padding:4px 12px;cursor:pointer;font-size:12px">× حذف</button>
    </div>
    <div style="display:grid;grid-template-columns:1fr;gap:8px">
      <div>
        <label style="display:block;font-size:13px;margin-bottom:4px;color:var(--weps-navy)">رابط المنتج (URL) <span style="color:var(--weps-orange)">*</span></label>
        <input type="url" class="product-url" placeholder="https://example.com/product" required style="width:100%;padding:8px;border:1px solid rgba(11,18,32,0.1);border-radius:6px;font-size:14px"/>
      </div>
      <div>
        <label style="display:block;font-size:13px;margin-bottom:4px;color:var(--weps-navy)">اسم المنتج</label>
        <input type="text" class="product-name" placeholder="اسم المنتج" style="width:100%;padding:8px;border:1px solid rgba(11,18,32,0.1);border-radius:6px;font-size:14px"/>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <div>
          <label style="display:block;font-size:13px;margin-bottom:4px;color:var(--weps-navy)">الكمية المطلوبة</label>
          <input type="number" class="product-quantity" min="1" value="1" style="width:100%;padding:8px;border:1px solid rgba(11,18,32,0.1);border-radius:6px;font-size:14px"/>
        </div>
        <div>
          <label style="display:block;font-size:13px;margin-bottom:4px;color:var(--weps-navy)">قيمة المنتج (USD) <span style="color:var(--weps-orange)">*</span></label>
          <input type="number" class="product-value" min="0" step="0.01" required style="width:100%;padding:8px;border:1px solid rgba(11,18,32,0.1);border-radius:6px;font-size:14px"/>
        </div>
      </div>
    </div>
  `;
  
  purchaseProductsList.appendChild(productRow);
...
function updateProductNumbers() {
  if (!purchaseProductsList) return;
  const items = purchaseProductsList.querySelectorAll('.purchase-product-item');
  items.forEach((item, index) => {
    const label = item.querySelector('label');
    if (label) {
      label.textContent = `منتج #${index + 1}`;
    }
  });
}
...
function getPurchaseProducts() {
  ...
  items.forEach((item, index) => {
    const url = item.querySelector('.product-url')?.value?.trim();
    const name = item.querySelector('.product-name')?.value?.trim() || `منتج #${index + 1}`;
    ...
    if (url && value > 0) {
      products.push({ url, name, quantity, value });
    }
  });
...
// Initialize with one product row if purchase section is shown
if (purchaseProductsList && purchaseProductsList.children.length === 0) {
  // Will be added when purchase option is selected
}
...
quickCalcBtn.addEventListener('click', () => {
  ...
  quickResultBody.innerHTML = `
    ...
    <ul style="margin:0 0 12px 20px;padding:0 18px;color:#6b7280;line-height:1.7">
      <li>التقدير يعتمد على خدمة <strong>Weps Express</strong> (10-14 أيام)</li>
...
function renderComparisonTable(serviceQuotes, options = {}) {
  ...
  const serviceTimes = {
    ...
    wepsAir: '40–45 يوم',
    wepsSea: '50–60 يوم',
    [SERVICES.EXPRESS_YEMEN]: '10-14 أيام',
    [SERVICES.AIR_ECO_UAE_OMAN_TRUCK_YEMEN]: '18-23 يوم',
    [SERVICES.AIR_ADEN_VIA_EG_JO]: '12-18 يوم',
    [SERVICES.SEA_KG_VIA_UAE_TRUCK_YEMEN]: '55-65 يوم',
    [SERVICES.SEA_CBM_VIA_UAE_TRUCK_YEMEN]: '35-45 يوم',
    [SERVICES.SEA_CBM_DIRECT_ADEN_MUKALLA]: '55-65 يوم'
  };
...
/* ===== Download Offer HTML ===== */
/* ===== Generate Full Report HTML (Shared function for download and Monday upload) ===== */
function generateFullReportHTML(options = {}) {
  const { returnAsString = false, forMonday = false } = options;
  ...
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/8d2fc7b0-c289-4711-8e2f-96141aa9ec6c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'script.js:5603', message: 'downloadOffer: serviceQuotes received', data: { serviceQuotesKeys: Object.keys(serviceQuotes), serviceQuotesCount: Object.keys(serviceQuotes).length, firstServiceKey: Object.keys(serviceQuotes)[0], firstServiceCustoms: serviceQuotes[Object.keys(serviceQuotes)[0]]?.customs }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'D' }) }).catch(() => { });
  // #endregion
  ...
  const serviceNameMap = {
    ...
    [SERVICES.AIR_ECO_UAE_OMAN_TRUCK_YEMEN]: 'جو اقتصادي الإمارات - بري اليمن',
  ...
  const reportCards = availableServiceOrder.map(entry => {
    ...
    if (!quote) return '';
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/8d2fc7b0-c289-4711-8e2f-96141aa9ec6c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'script.js:5648', message: 'reportCards map: quote customs check', data: { serviceKey: entry.key, hasQuote: !!quote, hasCustoms: !!quote?.customs, customsSuccess: quote?.customs?.success, customsTotalUSD: quote?.customs?.calculation?.total_customs_usd, customsRate: quote?.customs?.calculation?.final_rate_percent, customsFull: JSON.stringify(quote?.customs) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
    // #endregion
    ...
    const customsRate = quote.customs?.success && quote.customs.calculation.final_rate_percent != null
      ...
      : null;
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/8d2fc7b0-c289-4711-8e2f-96141aa9ec6c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'script.js:5665', message: 'reportCards map: calculated values', data: { serviceKey: entry.key, customsUSD: customsUSD, customsRate: customsRate, willShowCustoms: quote.customs?.success, displayValue: quote.customs?.success ? formatUSD(quote.customs.calculation.total_customs_usd) : '---' }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'B' }) }).catch(() => { });
    // #endregion
    ...
    return `
      <div class="report-service-card ${escapeHtml(entry.key)}" style="${cardStyle}">
        ...
        <div class="report-service-meta"><span>الجمارك${customsRate ? ` (${customsRate}%)` : ''}</span><strong>${quote.customs?.success ? (customsUSD !== null ? formatUSD(customsUSD) : '---') : (quote.customs?.error ? `err: ${quote.customs.error}` : '---')}</strong></div>
  ...
  const offerHTML = `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>تقرير الحل اللوجستي الذكي ${offerId}</title><style>
    ...
    ${vasCost && vasCost.totalCost > 0 ? `
    <div class="section">
      <h3>الخدمات الإضافية</h3>
      <div class="info-grid">
        ${vasCost.services.map(service => `
          <div class="info-card">
            <span class="label">${escapeHtml(service.label)}</span>
            <span class="value">${formatUSD(service.cost)}</span>
          </div>
        `).join('')}
        <div class="info-card" style="background:#fff7ed;border:2px solid #f58220">
          <span class="label">إجمالي الخدمات الإضافية</span>
          <span class="value" style="color:#f58220;font-size:18px">${formatUSD(vasCost.totalCost)}</span>
        </div>
      </div>
    </div>
    ` : ''}

    ${results.purchaseProducts && results.purchaseProducts.length > 0 ? `
    <div class="section">
      <h3>المنتجات المطلوب شراؤها</h3>
      <table>
        <thead>
          <tr>
            <th>#</th>
        ...
        <tbody>
          ${results.purchaseProducts.map((p, i) => `
            <tr>
              <td>${i + 1}</td>
              <td><a href="${escapeHtml(p.url)}" target="_blank" style="color:#f58220;text-decoration:underline">${escapeHtml(p.url)}</a></td>
  ...
  const cssStyles = `
    <style>
      body{font-family:"Cairo",Arial,Tahoma,sans-serif;background:#fff;margin:0;padding:18px;color:#0b1220;direction:rtl;text-align:right}
      .container{max-width:980px;margin:0 auto}
      .header{background:linear-gradient(90deg,#fff 0%,#fff4e9 50%,#ffe3ca 100%);padding:22px;border-radius:12px;text-align:center}
      .logo{width:64px;height:64px;border-radius:12px;background:#0b1220;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-weight:800}
      .section{background:#fff;padding:16px;margin-top:12px;border-radius:10px;border:1px solid rgba(11,18,32,0.06)}
  ...
  // Body content (without html/head/body tags for Monday)
  const bodyContent = offerHTML.replace(/^<!doctype html><html lang="ar" dir="rtl"><head>.*?<\/head><body>/is, '').replace(/<\/body><\/html>$/is, '');

  // If for Monday, return content with CSS embedded
  if (forMonday) {
    return cssStyles + bodyContent;
  }

  // If returnAsString, return full HTML as string
  if (returnAsString) {
    return offerHTML;
  }
...
/* ===== Download Offer (uses generateFullReportHTML) ===== */
function downloadOffer() {
  generateFullReportHTML({ returnAsString: false, forMonday: false });
}
...
async function generateInvoicePDF() {
  ...
  tempDiv.innerHTML = `
    ...
    ${vasCost && vasCost.totalCost > 0 ? `
    <div style="margin-bottom:20px;padding:12px;background:#fff;border-radius:8px;border:1px solid rgba(11,18,32,0.06)">
      <h3 style="font-size:16px;font-weight:bold;margin-bottom:10px">الخدمات الإضافية</h3>
      <div style="font-size:12px;line-height:1.8">
        ${vasCost.services.map(service => `
          <div><strong>${service.label}:</strong> ${formatUSD(service.cost)}</div>
        `).join('')}
        <div style="margin-top:8px;padding:8px;background:#fff7ed;border-radius:6px;border-right:3px solid #f58220">
          <strong>إجمالي الخدمات الإضافية:</strong> <span style="color:#f58220;font-size:16px">${formatUSD(vasCost.totalCost)}</span>
        </div>
      </div>
    </div>
    ` : ''}
...
async function computeAndRender() {
  try {
    ...
    // Calculate VAS cost
    const vasCost = calculateVASCost();
    const selectedVAS = getSelectedVASKeys();
    ...
    // Calculate customs for all services
    Object.keys(serviceResults).forEach(serviceCode => {
      const result = serviceResults[serviceCode];
      if (result && cargoValue && productTypeAr && destCity) {
        try {
          const customsResult = calculateCustomsDuties(cargoValue, productTypeAr, destCity);
          if (customsResult) {
            result.customs = {
              success: true,
              calculation: {
                total_customs_usd: customsResult.finalPayableUSD,
                final_rate_percent: ((customsResult.finalPayableUSD / cargoValue) * 100),
                south_usd: customsResult.south.payableUSD,
                north_usd: customsResult.north ? customsResult.north.payableUSD : 0,
                is_north: customsResult.isNorth
              }
            };
            // Update total price to include customs
            result.totalPrice = (result.shippingPrice || 0) +
              (result.insurance?.success ? result.insurance.calculation.final_premium : 0) +
              customsResult.finalPayableUSD +
              (vasCost ? vasCost.totalCost : 0);
          }
        } catch (error) {
          console.warn(`Error calculating customs for service ${serviceCode}:`, error);
          result.customs = {
            success: false,
            error: error.message || 'حدث خطأ في حساب الجمارك'
          };
        }
      }
    });
    ...
    const serviceInfo = {
      [SERVICES.EXPRESS_YEMEN]: {
        name: 'ويبس اكسبريس (10-14 أيام) - من دبي فقط',
        icon: '🚀',
        color: '#f58220',
        time: '10-14 أيام'
      },
      [SERVICES.AIR_ECO_UAE_OMAN_TRUCK_YEMEN]: {
        name: 'جو اقتصادي الإمارات - بري اليمن (18-23 يوم)',
        icon: '✈️',
        color: '#0066cc',
        time: '18-23 يوم'
      },
      ...
      [SERVICES.SEA_KG_VIA_UAE_TRUCK_YEMEN]: {
        name: 'بحر (كجم) عبر الإمارات - بري اليمن (55-65 يوم)',
        icon: '🚢',
        color: '#0097a7',
        time: '55-65 يوم'
      },
      [SERVICES.SEA_CBM_VIA_UAE_TRUCK_YEMEN]: {
        name: 'بحر (م³) مباشر عدن/المكلا (35-45 يوم)',
        icon: '⛴️',
        color: '#00796b',
        time: '35-45 يوم'
      },
      [SERVICES.SEA_CBM_DIRECT_ADEN_MUKALLA]: {
        name: 'بحر (م³) عبر الإمارات - بري اليمن (55-65 يوم)',
        icon: '🚤',
        color: '#004d40',
        time: '55-65 يوم'
      }
    ...
    Object.keys(serviceResults).forEach(serviceCode => {
      ...
      // Use customs data from service result
      ...
      if (vasCost && vasCost.totalCost > 0) coverageParts.push('الخدمات الإضافية');
      ...
      servicesHTML += `
        <div class="service-option-card" style="border-color:${info.color};${borderStyle};position:relative">
          ...
          <div class="price" style="font-size:24px;margin:10px 0;color:${info.color}">$${priceUSD}</div>
          <div class="small" style="color:#6b7280;margin-bottom:6px">(${priceYER} ريال) يشمل ${coverageText}</div>
          ...
          ${customsSummary ? `<div class="small" style="color:#d84315">⚖️ الجمارك (${customsSummary.final_rate_percent.toFixed(2)}%): $${customsSummary.total_customs_usd.toFixed(2)}</div>` : `<div class="small" style="color:#d84315">⚖️ الجمارك: ${result?.customs?.error || 'سيتم تقديرها لاحقاً'}</div>`}
          ${vasCost && vasCost.totalCost > 0 ? `<div class="small" style="color:#f58220">✨ الخدمات الإضافية: ${formatUSD(vasCost.totalCost)}</div>` : ''}
        </div>
      `;
    });
    ...
    if (waDiscussBtn) {
      ...
      // Update WhatsApp link with new number and offer ID message
      const offerMessage = encodeURIComponent(`مرحباً، أود مناقشة العرض رقم: ${offerId}`);
      waDiscussBtn.href = `https://wa.me/967778666274?text=${offerMessage}`;
      waDiscussBtn.target = '_blank';
    }
    ...
    // Submit data to Monday.com new board (non-blocking)
    try {
      await submitToMondayNewBoard();
      console.log('✅ Data submitted to Monday.com successfully');
    } catch (mondayErr) {
      // Don't block the calculation if Monday submission fails
      console.error('Failed to submit to Monday.com:', mondayErr);
      // Optionally show a subtle notification (not blocking)
      // showToast('warning', 'تنبيه', 'تم الحساب بنجاح ولكن فشل حفظ البيانات في Monday.com');
    }
...
async function submitSurveyToMonday(formData, surveyData) {
  ...
  // Get selected service and calculate shipping + customs cost
  let shippingAndCustomsTotal = '';
  let selectedServiceName = '';

  // Try to get from stored results first
  if (window.__lastCalculationResults) {
    const results = window.__lastCalculationResults;
    const selectedServiceCode = results.selectedService || 
                                document.getElementById('serviceType')?.value || 
                                formData.serviceType || 
                                SERVICES.EXPRESS_YEMEN;
    
    const selectedServiceResult = results.serviceResults?.[selectedServiceCode];
    
    if (selectedServiceResult) {
      // Calculate shipping + customs (without insurance and VAS)
      const shippingPrice = selectedServiceResult.shippingPrice || 0;
      const customsUSD = selectedServiceResult.customs?.calculation?.total_customs_usd || 0;
      shippingAndCustomsTotal = (shippingPrice + customsUSD).toFixed(2);
      
      // Get service name from serviceInfo mapping
      const serviceInfo = {
        [SERVICES.EXPRESS_YEMEN]: 'ويبس اكسبريس (10-14 أيام) - من دبي فقط',
        [SERVICES.AIR_ECO_UAE_OMAN_TRUCK_YEMEN]: 'جو اقتصادي الإمارات - بري اليمن (18-23 يوم)',
        [SERVICES.AIR_ADEN_VIA_EG_JO]: 'جو عدن عبر مصر/الأردن (12-18 يوم)',
        [SERVICES.SEA_KG_VIA_UAE_TRUCK_YEMEN]: 'بحر (كجم) عبر الإمارات - بري اليمن (55-65 يوم)',
        [SERVICES.SEA_CBM_VIA_UAE_TRUCK_YEMEN]: 'بحر (م³) مباشر عدن/المكلا (35-45 يوم)',
        [SERVICES.SEA_CBM_DIRECT_ADEN_MUKALLA]: 'بحر (م³) عبر الإمارات - بري اليمن (55-65 يوم)'
      };
      
      selectedServiceName = serviceInfo[selectedServiceCode] || formData.serviceTypeText || selectedServiceCode;
    }
  } else if (formData.serviceTypeText) {
    // Fallback to form data
    selectedServiceName = formData.serviceTypeText;
  }
  ...
  // Add shipping + customs total (without insurance and VAS) to text_mkyd5pt2
  if (shippingAndCustomsTotal) {
    columnValues.text_mkyd5pt2 = shippingAndCustomsTotal;
  }
  
  // Add selected service name
  if (selectedServiceName) {
    columnValues.text_mkz4a0gm = selectedServiceName;
  }
  ...
  if (window.__lastCalculationResults) {
    try {
      // Use generateFullReportHTML with forMonday=true to get full report HTML for Monday
      reportHTML = generateFullReportHTML({ returnAsString: true, forMonday: true });
      if (!reportHTML) {
        reportHTML = 'لا توجد بيانات كافية لتوليد التقرير.';
      }
    } catch (err) {
      console.error('Error generating report HTML:', err);
      reportHTML = `خطأ في توليد التقرير: ${err.message}`;
    }
  }
  
  // Add report HTML to columnValues
  if (reportHTML) {
    columnValues.long_text_mkygqv7k = reportHTML;
  }

  // Check if we have an existing item ID to update, otherwise create new
  const existingItemId = window.__mondayItemId || null;
  ...
  try {
    ...
    // Save item ID if it's a new item
    if (!existingItemId && result.data?.create_item?.id) {
      window.__mondayItemId = result.data.create_item.id;
    }
...
// Function to submit data to Monday.com new board (18391595064)
async function submitToMondayNewBoard() {
  console.log('═══════════════════════════════════════════════════');
  console.log('🚀 [MONDAY UPLOAD] بدء عملية رفع البيانات إلى Monday.com');
  console.log('═══════════════════════════════════════════════════');
  
  const token = getMondayToken();
  if (!token) {
    console.error('❌ [MONDAY UPLOAD] مفتاح API غير متوفر');
    throw new Error('مفتاح Monday API غير متوفر.');
  }
  console.log('✅ [MONDAY UPLOAD] تم الحصول على API token');
  ...
  console.log('📋 [MONDAY UPLOAD] البيانات المجمعة:');
  console.log('  - اسم العميل:', customerName);
  console.log('  - الجوال:', customerPhone);
  console.log('  - نوع العميل:', customerType);

  if (!customerName || !customerPhone) {
    console.error('❌ [MONDAY UPLOAD] بيانات ناقصة');
    throw new Error('بيانات العميل غير مكتملة (الاسم والجوال مطلوبان).');
  }

  // Generate report HTML for Monday (full HTML with DOCTYPE)
  let reportHTML = '';
  if (window.__lastCalculationResults) {
    try {
      console.log('📄 [MONDAY UPLOAD] توليد تقرير HTML...');
      // Get full HTML report (not the Monday-embedded version)
      reportHTML = generateFullReportHTML({ returnAsString: true, forMonday: false });
      if (!reportHTML) {
        console.warn('⚠️ [MONDAY UPLOAD] لم يتم توليد HTML');
      } else {
        console.log('✅ [MONDAY UPLOAD] تم توليد HTML بنجاح (الحجم:', reportHTML.length, 'أحرف)');
      }
    } catch (err) {
      console.error('❌ [MONDAY UPLOAD] خطأ في توليد HTML:', err);
      reportHTML = '';
    }
  } else {
    console.warn('⚠️ [MONDAY UPLOAD] لا توجد نتائج حساب متاحة');
  }

  // Prepare column values for Monday.com
  const columnValues = {};
  
  // Map data to Monday columns
  columnValues.text_mkydq4n9 = customerName;  // اسم العميل
  columnValues.text_mkydv0ga = customerPhone; // جوال
  if (customerType) {
    columnValues.text_mkydq1yg = getCustomerTypeLabel(customerType); // نوع العميل
  }

  console.log('📊 [MONDAY UPLOAD] قيم الأعمدة:', columnValues);

  // Step 1: Create item with text data
  const itemName = `عرض - ${customerName} - ${new Date().toLocaleDateString('ar-EG')}`;
  console.log('📝 [MONDAY UPLOAD] اسم العنصر:', itemName);
  ...
  console.log('🔄 [MONDAY UPLOAD] Board ID:', createVariables.boardId);

  try {
    // Create the item first
    console.log('📤 [MONDAY UPLOAD] إرسال طلب إنشاء العنصر...');
    ...
    console.log('📥 [MONDAY UPLOAD] استجابة الخادم:', createResponse.status, createResponse.statusText);

    const createRawText = await createResponse.text();
    let createResult;

    try {
      createResult = JSON.parse(createRawText);
      console.log('📋 [MONDAY UPLOAD] نتيجة الاستجابة:', createResult);
    } catch (parseErr) {
      console.error('❌ [MONDAY UPLOAD] خطأ في تحليل الاستجابة:', parseErr, createRawText);
      throw new Error('استجابة غير متوقعة من Monday.');
    }

    if (!createResponse.ok) {
      const message = Array.isArray(createResult?.errors)
        ? createResult.errors.map(e => e?.message).filter(Boolean).join(' | ')
        : createResponse.statusText;
      console.error('❌ [MONDAY UPLOAD] فشل الطلب:', message);
      throw new Error(`HTTP ${createResponse.status}: ${message || 'خطأ غير معروف من Monday'}`);
    }

    if (Array.isArray(createResult.errors) && createResult.errors.length) {
      const errorMsg = createResult.errors.map(e => e?.message || 'خطأ غير معروف').join(' | ');
      console.error('❌ [MONDAY UPLOAD] أخطاء GraphQL:', errorMsg);
      throw new Error(errorMsg);
    }

    const itemId = createResult.data?.create_item?.id;
    if (!itemId) {
      console.error('❌ [MONDAY UPLOAD] لم يتم إنشاء العنصر');
      throw new Error('لم يتم إنشاء العنصر في Monday.');
    }

    console.log('✅✅✅ [MONDAY UPLOAD] تم إنشاء العنصر بنجاح!');
    console.log('🆔 [MONDAY UPLOAD] Item ID:', itemId);
    
    // Show success toast for item creation
    showToast('success', 'تم الحفظ', `تم حفظ البيانات في Monday.com (ID: ${itemId})`);

    // Step 2: Save HTML report as update (comment) on the item
    if (reportHTML) {
      console.log('───────────────────────────────────────────────────');
      console.log('📎 [MONDAY UPLOAD] حفظ تقرير HTML في Monday.com...');
      console.log('───────────────────────────────────────────────────');
      try {
        await saveHTMLReportToMonday(itemId, reportHTML, token, customerName);
        console.log('✅✅✅ [MONDAY UPLOAD] تم حفظ التقرير بنجاح!');
        
        // Also download the report locally for user convenience
        console.log('💾 [MONDAY UPLOAD] تحميل التقرير محلياً...');
        downloadHTMLReport(reportHTML, customerName);
        
        // Show success toast
        showToast('success', 'تم الحفظ والتحميل', 'تم حفظ التقرير في Monday.com وتحميله على جهازك');
        
      } catch (reportErr) {
        console.error('❌❌❌ [MONDAY UPLOAD] فشل حفظ التقرير:', reportErr);
        console.error('📋 [MONDAY UPLOAD] تفاصيل الخطأ:', {
          message: reportErr.message,
          stack: reportErr.stack,
          name: reportErr.name
        });
        
        // Still try to download locally even if Monday save failed
        try {
          console.log('💾 [MONDAY UPLOAD] محاولة التحميل المحلي فقط...');
          downloadHTMLReport(reportHTML, customerName);
          showToast('warning', 'تنبيه', 'تم حفظ البيانات وتحميل التقرير محلياً، لكن فشل حفظ التقرير في Monday.com');
        } catch (downloadErr) {
          console.error('❌ [MONDAY UPLOAD] فشل التحميل المحلي:', downloadErr);
          showToast('warning', 'تنبيه', 'تم حفظ البيانات في Monday.com فقط');
        }
      }
    } else {
      console.warn('⚠️ [MONDAY UPLOAD] لا يوجد تقرير HTML لحفظه');
      showToast('warning', 'تنبيه', 'تم حفظ البيانات ولكن لم يتم توليد التقرير');
    }

    console.log('═══════════════════════════════════════════════════');
    console.log('✅ [MONDAY UPLOAD] اكتملت عملية الرفع');
    console.log('═══════════════════════════════════════════════════');

    return { success: true, itemId: itemId };

  } catch (err) {
    console.error('❌❌❌ [MONDAY UPLOAD] خطأ عام:', err);
    showToast('error', 'خطأ', `فشل الحفظ في Monday.com: ${err.message}`);
    throw err;
  }
}

// Helper function to generate complete HTML report with tables for Monday.com updates
function generateMondayHTMLReport() {
  const results = window.__lastCalculationResults;
  
  if (!results) {
    return '<p>لا توجد بيانات متاحة لتوليد التقرير.</p>';
  }
  ...
  // Service information - Complete mapping for all service keys
  const SERVICE_KEY_MAPPING = {
    'express_yemen': { displayName: 'ويبس اكسبريس (10-14 أيام) - من دبي فقط', time: '10-14 أيام' },
    'air_eco_uae_oman_truck_yemen': { displayName: 'جو اقتصادي الإمارات - بري اليمن (18-23 يوم)', time: '18-23 يوم' },
    'air_aden_via_eg_jo': { displayName: 'جو عدن عبر مصر/الأردن (12-18 يوم)', time: '12-18 يوم' },
    'sea_kg_via_uae_truck_yemen': { displayName: 'بحر (كجم) عبر الإمارات - بري اليمن (55-65 يوم)', time: '55-65 يوم' },
    'sea_cbm_via_uae_truck_yemen': { displayName: 'بحر (م³) مباشر عدن/المكلا (35-45 يوم)', time: '35-45 يوم' },
    'sea_cbm_direct_aden_mukalla': { displayName: 'بحر (م³) عبر الإمارات - بري اليمن (55-65 يوم)', time: '55-65 يوم' },
    // Legacy keys support
    'wepsExpress': { displayName: 'ويبس اكسبريس (10-14 أيام)', time: '10-14 أيام' },
    'wepsAir': { displayName: 'جو اقتصادي (18-23 يوم)', time: '18-23 يوم' },
    'wepsSea': { displayName: 'بحر اقتصادي (55-65 يوم)', time: '55-65 يوم' }
  };

  // Simplified HTML for Monday.com compatibility (no CSS styles)
  let html = '';
  
  // Header - Simplified
  html += `<h2>📋 تقرير الحل اللوجستي الذكي</h2>`;
  html += `<p><strong>رقم العرض:</strong> ${offerId || '---'}</p>`;
  html += `<p><strong>التاريخ:</strong> ${new Date().toLocaleDateString('ar-EG', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric',
    hour: '2-digit',
  ...
  // Customer Information
  html += '<h3>👤 معلومات العميل</h3>';
  html += '<table>';
  html += '<tr><th>البيان</th><th>القيمة</th></tr>';
  html += `<tr><td>الاسم</td><td>${customerName || '---'}</td></tr>`;
  html += `<tr><td>الجوال</td><td>${customerPhone || '---'}</td></tr>`;
  html += `<tr><td>نوع العميل</td><td>${getCustomerTypeLabel(customerType) || '---'}</td></tr>`;
  if (companyName) {
    html += `<tr><td>المؤسسة/الشركة</td><td>${companyName}</td></tr>`;
  }
  html += '</table>';

  // Shipment Details
  html += '<h3>📦 تفاصيل الشحنة</h3>';
  html += '<table>';
  html += '<tr><th>البيان</th><th>القيمة</th></tr>';
  html += `<tr><td>الدولة المصدرة</td><td>${originCountry || '---'}</td></tr>`;
  html += `<tr><td>الوجهة (المدينة)</td><td>${destCity || '---'}</td></tr>`;
  html += `<tr><td>نوع المنتج</td><td>${productTypeAr || '---'}</td></tr>`;
  if (productCategory) {
    html += `<tr><td>فئة المنتج</td><td>${productCategory}</td></tr>`;
  }
  if (productDescription) {
    html += `<tr><td>وصف البضاعة</td><td>${productDescription}</td></tr>`;
  }
  html += `<tr><td>الوزن الإجمالي</td><td>${totalWeight || '---'} كجم</td></tr>`;
  html += `<tr><td>قيمة البضاعة</td><td>$${cargoValue || '---'}</td></tr>`;
  html += `<tr><td>رقم العرض</td><td>${offerId || '---'}</td></tr>`;
  html += '</table>';

  // Supplier Information
  if (supplierName || supplierContact || supplierCountry || supplierAddress || supplierEmail || supplierPhone) {
    html += '<h3>🏭 معلومات المصنع / المورد</h3>';
    html += '<table>';
    html += '<tr><th>البيان</th><th>القيمة</th></tr>';
    if (supplierName) html += `<tr><td>اسم المصنع / المورد</td><td>${supplierName}</td></tr>`;
    if (supplierContact) html += `<tr><td>شخص التواصل لدى المورد</td><td>${supplierContact}</td></tr>`;
    if (supplierCountry) html += `<tr><td>دولة المصنع / المورد</td><td>${supplierCountry}</td></tr>`;
    if (supplierAddress) html += `<tr><td>عنوان المصنع / المورد</td><td>${supplierAddress}</td></tr>`;
    if (supplierEmail) html += `<tr><td>البريد الإلكتروني للمورد</td><td>${supplierEmail}</td></tr>`;
    if (supplierPhone) html += `<tr><td>هاتف المورد</td><td>${supplierPhone}</td></tr>`;
    html += '</table>';
  }

  // Purchase Products Information
  if (purchaseProducts && purchaseProducts.length > 0) {
    html += '<h3>🛒 المنتجات المشتراة</h3>';
    html += '<table>';
    html += '<tr><th>#</th><th>اسم المنتج</th><th>رابط المنتج</th><th>الكمية</th><th>القيمة (USD)</th></tr>';
    purchaseProducts.forEach((product, idx) => {
      const productName = product.name || product.productName || '---';
      const productLink = product.link || product.url || product.productLink || '---';
      const quantity = product.quantity || product.qty || 1;
      const value = product.value || product.price || product.totalPrice || 0;
      
      html += `<tr>`;
      html += `<td>${idx + 1}</td>`;
      html += `<td>${productName}</td>`;
      html += `<td>${productLink !== '---' ? `<a href="${productLink}" target="_blank">رابط</a>` : '---'}</td>`;
      html += `<td>${quantity}</td>`;
  ...
  if (parcels && parcels.length > 0) {
    html += '<h3>📦 تفاصيل الطرود</h3>';
    html += '<table>';
    html += '<tr><th>#</th><th>الطول (سم)</th><th>العرض (سم)</th><th>الارتفاع (سم)</th><th>الوزن الفعلي (كجم)</th><th>الوزن الحجمي (كجم)</th><th>الكمية</th></tr>';
    parcels.forEach((parcel, idx) => {
      html += `<tr>`;
      html += `<td>${idx + 1}</td>`;
      html += `<td>${parcel.length || '---'}</td>`;
      html += `<td>${parcel.width || '---'}</td>`;
      html += `<td>${parcel.height || '---'}</td>`;
      html += `<td>${parcel.actual || '---'}</td>`;
      html += `<td>${parcel.volWeight?.toFixed(2) || '---'}</td>`;
      html += `<td>${parcel.quantity || 1}</td>`;
      html += `</tr>`;
    });
    html += '</table>';
  }

  // Selected Service Details - Find selected service flexibly
  ...
  if (serviceResults) {
    ...
    if (serviceResults[selectedService]) {
      selectedServiceData = serviceResults[selectedService];
      selectedServiceKey = selectedService;
      selectedServiceInfo = SERVICE_KEY_MAPPING[selectedService];
    } else {
      // Try to find by looking through all available services
      const availableKeys = Object.keys(serviceResults);
      if (availableKeys.length > 0) {
        // Use the first available service as fallback
        selectedServiceKey = availableKeys[0];
        selectedServiceData = serviceResults[selectedServiceKey];
        selectedServiceInfo = SERVICE_KEY_MAPPING[selectedServiceKey];
      }
    }
  }
  
  if (selectedServiceData && selectedServiceInfo) {
    html += '<h3>✅ الخدمة المختارة</h3>';
    html += '<table>';
    html += '<tr><th>البيان</th><th>القيمة</th></tr>';
    html += `<tr><td>الخدمة المختارة</td><td>${selectedServiceInfo.displayName}</td></tr>`;
    html += `<tr><td>وقت التوصيل</td><td>${selectedServiceInfo.time}</td></tr>`;
    html += `<tr><td><strong>السعر الإجمالي</strong></td><td><strong>$${(selectedServiceData.totalPrice || 0).toFixed(2)} (${((selectedServiceData.totalPrice || 0) * 530).toFixed(0)} ريال)</strong></td></tr>`;
    html += `<tr><td>تكلفة الشحن</td><td>$${(selectedServiceData.shippingPrice || 0).toFixed(2)}</td></tr>`;
    html += `<tr><td>الجمارك</td><td>$${(selectedServiceData.customs?.calculation?.total_customs_usd || 0).toFixed(2)}</td></tr>`;
    const insuranceClause = selectedServiceData.insurance?.calculation?.insurance_clause || 'Clause B';
    html += `<tr><td>التأمين (${insuranceClause})</td><td>$${(selectedServiceData.insurance?.calculation?.final_premium || 0).toFixed(2)}</td></tr>`;
    if (vasCost > 0) {
      html += `<tr><td>إجمالي الخدمات الإضافية</td><td>$${vasCost.toFixed(2)}</td></tr>`;
    }
    html += '</table>';
  }

  // Customs Details
  if (selectedServiceData?.customs?.calculation) {
    const customs = selectedServiceData.customs.calculation;
    html += '<h3>🛃 تفاصيل الجمارك</h3>';
    html += '<table>';
    html += '<tr><th>البيان</th><th>القيمة</th></tr>';
    html += `<tr><td>إجمالي الجمارك</td><td>$${(customs.total_customs_usd || 0).toFixed(2)}</td></tr>`;
    html += `<tr><td>النسبة المئوية</td><td>${(customs.final_rate_percent || 0).toFixed(2)}%</td></tr>`;
    if (customs.south_usd) {
      html += `<tr><td>جمارك الجنوب</td><td>$${customs.south_usd.toFixed(2)}</td></tr>`;
    }
    if (customs.north_usd) {
      html += `<tr><td>جمارك الشمال</td><td>$${customs.north_usd.toFixed(2)}</td></tr>`;
    }
    html += `<tr><td>المنطقة</td><td>${customs.is_north ? 'الشمال' : 'الجنوب'}</td></tr>`;
    html += '</table>';
  }

  // Insurance Details
  if (selectedServiceData?.insurance?.calculation) {
    const insurance = selectedServiceData.insurance.calculation;
    html += '<h3>🛡️ تفاصيل التأمين</h3>';
    html += '<table>';
    html += '<tr><th>البيان</th><th>القيمة</th></tr>';
    html += `<tr><td>قسط التأمين</td><td>$${(insurance.final_premium || 0).toFixed(2)}</td></tr>`;
    html += `<tr><td>نوع التأمين</td><td>${insurance.insurance_clause || '---'}</td></tr>`;
    html += `<tr><td>النسبة</td><td>${(insurance.rate_percent || 0).toFixed(2)}%</td></tr>`;
    html += '</table>';
  }

  // Additional Services (VAS) - Detailed
  if (selectedVAS && selectedVAS.length > 0) {
    html += '<h3>🎁 الخدمات الإضافية</h3>';
    html += '<table>';
    html += '<tr><th>الخدمة</th><th>الوصف</th></tr>';
    selectedVAS.forEach(vas => {
      let description = '';
      if (vas.includes('صور') || vas.includes('photo') || vas.toLowerCase().includes('photo')) {
        description = 'صور للشحنة - تصوير شامل للشحنة قبل الإرسال';
      } else if (vas.includes('فحص') || vas.includes('inspect') || vas.toLowerCase().includes('inspection')) {
        description = 'فحص شامل - فحص دقيق للمنتجات قبل الشحن';
      } else if (vas.includes('شراء') || vas.includes('purchase') || vas.toLowerCase().includes('purchase')) {
        description = 'شراء المنتج - خدمة شراء المنتجات نيابة عنك';
      } else {
        description = 'خدمة إضافية';
      }
      html += `<tr><td>${vas}</td><td>${description}</td></tr>`;
    });
    if (vasCost > 0) {
      html += `<tr><td><strong>إجمالي الخدمات الإضافية</strong></td><td><strong>$${vasCost.toFixed(2)}</strong></td></tr>`;
    }
    html += '</table>';
  }
  ...
  if (supplierName || supplierContact) {
    html += '<h3>🏭 معلومات المورد</h3>';
    html += '<table>';
    html += '<tr><th>البيان</th><th>القيمة</th></tr>';
    if (supplierName) html += `<tr><td>اسم المورد</td><td>${supplierName}</td></tr>`;
    if (supplierContact) html += `<tr><td>جهة الاتصال</td><td>${supplierContact}</td></tr>`;
    if (supplierCountry) html += `<tr><td>الدولة</td><td>${supplierCountry}</td></tr>`;
    if (supplierAddress) html += `<tr><td>العنوان</td><td>${supplierAddress}</td></tr>`;
    if (supplierEmail) html += `<tr><td>البريد الإلكتروني</td><td>${supplierEmail}</td></tr>`;
    if (supplierPhone) html += `<tr><td>الهاتف</td><td>${supplierPhone}</td></tr>`;
    html += '</table>';
  }

  // Footer - Simplified
  html += '<hr>';
  html += '<p><strong>شركة ويبس لخدمات الشحن الدولي</strong></p>';
  html += '<p>هذا التقرير تم إنشاؤه تلقائياً بواسطة نظام حاسبة الأسعار الذكية</p>';
  html += `<p>📅 ${new Date().toLocaleDateString('ar-EG')} | 🆔 ${offerId || '---'}</p>`;

  return html;
}

// OLD TEXT REPORT FUNCTION - Kept for reference
// Helper function to generate a formatted text report for Monday.com updates
function generateMondayUpdateReport() {
  const results = window.__lastCalculationResults;
  
  if (!results) {
    return 'لا توجد بيانات متاحة لتوليد التقرير.';
  }
  ...
  // Service names and delivery times
  const serviceInfo = {
    'wepsExpress': { name: 'ويبس اكسبريس', time: '10-14 أيام', icon: '🚀' },
    'wepsAir': { name: 'جو اقتصادي', time: '18-23 يوم', icon: '✈️' },
    'wepsSea': { name: 'بحر اقتصادي', time: '55-65 يوم', icon: '🚢' }
  };

  // Get selected service info
  const selectedServiceInfo = serviceResults?.[selectedService];
  const selectedServiceName = serviceInfo[selectedService]?.name || 'غير محدد';
  
  // Build the report
  let report = '';
  
  // Header
  report += '📋 تقرير الحل اللوجستي الذكي\n';
  report += '═══════════════════════════════════════════════════\n\n';
  
  // Customer Information
  report += '👤 معلومات العميل\n';
  report += '─────────────────────────\n';
  report += `├─ الاسم: ${customerName || '---'}\n`;
  report += `├─ الجوال: ${customerPhone || '---'}\n`;
  report += `├─ النوع: ${getCustomerTypeLabel(customerType) || '---'}\n`;
  if (companyName) {
    report += `└─ المؤسسة: ${companyName}\n`;
  } else {
    report += `└─ المؤسسة: ---\n`;
  }
  report += '\n';
  ...
  // Shipment Details
  report += '📦 تفاصيل الشحنة\n';
  report += '─────────────────────────\n';
  report += `├─ من: ${originCountry || '---'}\n`;
  report += `├─ إلى: ${destCity || '---'}\n`;
  report += `├─ نوع البضاعة: ${productTypeAr || '---'}\n`;
  report += `├─ الوزن الإجمالي: ${totalWeight || '---'} كجم\n`;
  report += `└─ قيمة البضاعة: $${cargoValue || '---'}\n`;
  report += '\n';
  
  // Services Section
  report += '💰 الخدمات المتاحة\n';
  report += '═══════════════════════════════════════════════════\n\n';
  
  // Loop through available services
  Object.keys(serviceInfo).forEach(serviceKey => {
    const service = serviceInfo[serviceKey];
    const serviceData = serviceResults?.[serviceKey];
    
    if (serviceData) {
      const isSelected = serviceKey === selectedService;
      
      report += `${service.icon} ${service.name} (${service.time})${isSelected ? ' ✅ المختار' : ''}\n`;
      report += '─────────────────────────\n';
      
      const totalPrice = serviceData.totalPrice || 0;
      const shippingPrice = serviceData.shippingPrice || 0;
      const customsUSD = serviceData.customs?.calculation?.total_customs_usd || 0;
      const insuranceUSD = serviceData.insurance?.calculation?.final_premium || 0;
      const vasPrice = vasCost || 0;
      
      report += `   💵 السعر الإجمالي: $${totalPrice.toFixed(2)} (${(totalPrice * 530).toFixed(0)} ريال)\n`;
      report += `   • الشحن: $${shippingPrice.toFixed(2)}\n`;
      report += `   • الجمارك: $${customsUSD.toFixed(2)}\n`;
      report += `   • التأمين: $${insuranceUSD.toFixed(2)}\n`;
      if (vasPrice > 0) {
        report += `   • خدمات إضافية: $${vasPrice.toFixed(2)}\n`;
      }
      report += '\n';
    }
  });
  ...
  if (selectedServiceInfo) {
    report += '═══════════════════════════════════════════════════\n';
    report += '✅ الخدمة المختارة\n';
    report += '─────────────────────────\n';
    report += `📌 الخدمة: ${selectedServiceName}\n`;
    report += `💵 السعر النهائي: $${(selectedServiceInfo.totalPrice || 0).toFixed(2)}\n`;
    report += `💰 بالريال اليمني: ${((selectedServiceInfo.totalPrice || 0) * 530).toFixed(0)} ريال\n`;
    report += '\n';
  }
  
  // Additional Services (VAS)
  if (selectedVAS && selectedVAS.length > 0) {
    report += '🎁 الخدمات الإضافية المختارة\n';
    report += '─────────────────────────\n';
    selectedVAS.forEach(vas => {
      report += `   • ${vas}\n`;
    });
  ...
  report += `🆔 رقم العرض: ${offerId || '---'}\n`;
  report += '═══════════════════════════════════════════════════\n';
  
  return report;
}

// Helper function to save formatted report as update (comment) in Monday.com
async function saveHTMLReportToMonday(itemId, htmlContent, token, customerName) {
  console.log('📝 [SAVE REPORT] Saving HTML table report as update in Monday.com...');
  console.log('📄 Item ID:', itemId);
  
  // Generate complete HTML report with tables for Monday.com
  const htmlTableReport = generateMondayHTMLReport();
  
  console.log('📊 [SAVE REPORT] Generated HTML report with tables, length:', htmlTableReport.length);
  
  // Create update (comment) with HTML table report
  ...
  // Check report size before sending
  const MAX_SIZE = 100000; // 100KB limit
  const reportSize = htmlTableReport.length;
  console.log(`📏 [SAVE REPORT] Report size: ${reportSize} characters (${(reportSize/1024).toFixed(2)} KB)`);
  
  if (reportSize > MAX_SIZE) {
    console.warn(`⚠️ [SAVE REPORT] Report size exceeds limit: ${reportSize} > ${MAX_SIZE}`);
    throw new Error(`التقرير كبير جداً (${(reportSize/1024).toFixed(2)} KB). الحد الأقصى: ${(MAX_SIZE/1024).toFixed(2)} KB`);
  }
  
  const updateBody = htmlTableReport;
  ...
  console.log('📤 [SAVE REPORT] Sending update to Monday.com...');
  
  // Create AbortController for timeout
  const controller = new AbortController();
  const timeoutId = setTimeout(() => {
    controller.abort();
    console.error('⏱️ [SAVE REPORT] Request timeout after 30 seconds');
  ...
  try {
    const response = await fetch("https://api.monday.com/v2", {
      ...
      body: JSON.stringify({ query: createUpdateMutation, variables: variables }),
      signal: controller.signal
    });
    
    clearTimeout(timeoutId);
    
    console.log('📥 [SAVE REPORT] Response status:', response.status, response.statusText);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ [SAVE REPORT] Error response:', errorText);
      throw new Error(`فشل حفظ التقرير: HTTP ${response.status} - ${response.statusText}`);
    }
    
    const result = await response.json();
    console.log('✅ [SAVE REPORT] Monday.com response:', result);
    
    if (result.errors && result.errors.length > 0) {
      const errorMsg = result.errors.map(e => e?.message || 'خطأ غير معروف').join(' | ');
      console.error('❌ [SAVE REPORT] GraphQL errors:', errorMsg);
      throw new Error(`خطأ من Monday.com: ${errorMsg}`);
    }
    
    if (result.data?.create_update?.id) {
      console.log('✅✅✅ [SAVE REPORT] Update created successfully! Update ID:', result.data.create_update.id);
    } else {
      console.warn('⚠️ [SAVE REPORT] No update ID returned');
    }
    ...
    if (err.name === 'AbortError') {
      console.error('⏱️ [SAVE REPORT] Request timed out');
      throw new Error('انتهى وقت الطلب. التقرير قد يكون كبيراً جداً أو الاتصال بطيء');
    }
...
// Helper function to download HTML report locally
function downloadHTMLReport(htmlContent, customerName) {
  console.log('💾 [DOWNLOAD] Starting local download...');
  
  try {
    const offerId = window.__lastCalculationResults?.offerId || new Date().getTime();
    const fileName = `تقرير_الحل_اللوجستي_${customerName || 'عميل'}_${offerId}.html`;
    ...
    console.error('❌ [DOWNLOAD] Failed to download:', err);
    throw err;
  }
}

// OLD FUNCTION - Kept for reference but not used due to CORS
// Helper function to upload HTML file to Monday.com
async function uploadHTMLFileToMonday(itemId, htmlContent, token) {
  console.log('🚀 Starting file upload to Monday.com...');
  console.log('📄 Item ID:', itemId);
  console.log('📦 HTML content length:', htmlContent?.length || 0);
  
  const offerId = window.__lastCalculationResults?.offerId || new Date().getTime();
  // Use ASCII-safe filename to avoid encoding issues
  const fileName = `smart_logistics_report_${offerId}.html`;
  ...
  // Use Monday.com file upload API with proper FormData structure
  // Try method 1: /v2/file endpoint with map approach
  const formData = new FormData();
  
  // Add the GraphQL mutation query
  const mutation = `
    mutation add_file($file: File!) {
      add_file_to_column(
        item_id: ${itemId},
        column_id: "file_mkzck7v4",
        file: $file
      ) {
        id
        name
  ...
  // Map the file using Monday.com's required format
  const map = { "0": ["variables.file"] };
  formData.append('map', JSON.stringify(map));
  formData.append('0', file, fileName);
  
  console.log('📤 Sending file to Monday.com API...');
  console.log('🔗 Trying endpoint: https://api.monday.com/v2/file');

  try {
    // Try endpoint /v2/file first
    let response = await fetch("https://api.monday.com/v2/file", {
      method: "POST",
      headers: {
        "Authorization": token
    ...
    // If CORS error or network error, try /v2 endpoint
    if (!response.ok && response.status === 0) {
      console.log('⚠️ CORS issue detected, trying /v2 endpoint instead...');
      const retryFormData = new FormData();
      retryFormData.append('query', mutation);
      retryFormData.append('variables', JSON.stringify({ file: null }));
      retryFormData.append('map', JSON.stringify(map));
      retryFormData.append('0', file, fileName);
    ...
    if (!response.ok) {
      ...
      throw new Error(`فشل رفع الملف: HTTP ${response.status} - ${response.statusText}`);
    }
    ...
    try {
      ...
      throw new Error('استجابة غير متوقعة من Monday.com');
    }
    ...
    if (result.data?.add_file_to_column?.id) {
      ...
      console.log('📁 Asset ID:', result.data.add_file_to_column.id);
      console.log('📝 File name:', result.data.add_file_to_column.name);
      console.log('🔗 File URL:', result.data.add_file_to_column.url);
      return result;
    } else {
      console.warn('⚠️ No asset ID in response, but request succeeded');
      return result;
    }
    ...
    if (err.message.includes('Failed to fetch') || 
      ...
      const corsError = new Error('فشل الاتصال بـ Monday.com. قد يكون بسبب قيود CORS. يرجى التأكد من أن Monday.com يدعم رفع الملفات من المتصفح.');
...
/* ===== CUSTOMS CALCULATION FUNCTIONS ===== */

// Fetch exchange rates from Monday.com
async function fetchExchangeRatesFromMonday() {
  try {
    const token = getMondayToken();
    if (!token) {
      console.warn('⚠️ No Monday token available for exchange rates');
      return null;
    }

    const query = `
      query {
        boards(ids: [${BOARD_EXCHANGE_RATES}]) {
          items_page(limit: 1) {
            items {
              id
              name
    ...
    if (!items || items.length === 0) {
      console.warn('⚠️ No exchange rate data found in Monday.com');
      return null;
    }
    ...
    columnValues.forEach(col => {
      if (col.id === COLUMN_IDS_EXCHANGE_RATES.southMarketFx && col.text) {
        exchangeRates.southMarketFx = parseFloat(col.text) || 750;
      } else if (col.id === COLUMN_IDS_EXCHANGE_RATES.customsFxNorth && col.text) {
        exchangeRates.customsFxNorth = parseFloat(col.text) || 250;
      } else if (col.id === COLUMN_IDS_EXCHANGE_RATES.customsFxSouth && col.text) {
        exchangeRates.customsFxSouth = parseFloat(col.text) || 750;
      } else if (col.id === COLUMN_IDS_EXCHANGE_RATES.avgFxRate && col.text) {
        exchangeRates.avgFxRate = parseFloat(col.text) || 534;
      }
    });

    window.__WEPS_EXCHANGE_RATES = exchangeRates;
    console.log('✅ Exchange rates loaded from Monday.com:', exchangeRates);
    ...
    window.__WEPS_EXCHANGE_RATES = defaultRates;
    return defaultRates;
  }
}

// Fetch commodity types and their customs data from Monday.com
async function fetchCommodityTypesFromMonday() {
  try {
    const token = getMondayToken();
    if (!token) {
      console.warn('⚠️ No Monday token available for commodities');
      return [];
    }

    const query = `
      query {
        boards(ids: [${BOARD_COMMODITIES}]) {
    ...
    if (!items || items.length === 0) {
      console.warn('⚠️ No commodity data found in Monday.com');
      return [];
    }

    // Store commodity data
    const commoditiesData = items.map(item => {
      const commodityData = {
        id: item.id,
        name: item.name,
        commodityType: item.name,
        tariffSouth: 5,
        tariffNorth: 5,
        salesTax: 5,
        vat: 5,
      ...
      item.column_values.forEach(col => {
        if (col.id === COMMODITY_COLUMN_IDS.commodityType && col.text) {
          commodityData.commodityType = col.text;
        } else if (col.id === COMMODITY_COLUMN_IDS.tariffSouth && col.text) {
          commodityData.tariffSouth = parseFloat(col.text) || 5;
        } else if (col.id === COMMODITY_COLUMN_IDS.tariffNorth && col.text) {
          commodityData.tariffNorth = parseFloat(col.text) || 5;
        } else if (col.id === COMMODITY_COLUMN_IDS.salesTax && col.text) {
          commodityData.salesTax = parseFloat(col.text) || 5;
        } else if (col.id === COMMODITY_COLUMN_IDS.vat && col.text) {
          commodityData.vat = parseFloat(col.text) || 5;
        } else if (col.id === COMMODITY_COLUMN_IDS.profitTax && col.text) {
          commodityData.profitTax = parseFloat(col.text) || 3;
        }
      });
    ...
    window.__WEPS_COMMODITIES_DATA = commoditiesData;
    console.log(`✅ Loaded ${commoditiesData.length} commodity types from Monday.com`);
...
// Get commodity data by product type name
function getCommodityDataByProductType(productTypeName) {
  if (!productTypeName || !window.__WEPS_COMMODITIES_DATA) {
    return null;
  }

  const commodity = window.__WEPS_COMMODITIES_DATA.find(c =>
    c.commodityType === productTypeName || c.name === productTypeName
  );
...
// Calculate customs duties
function calculateCustomsDuties(cifUsd, productTypeName, destination) {
  // Get exchange rates
  const exchangeRates = window.__WEPS_EXCHANGE_RATES || {
    southMarketFx: 750,
    customsFxNorth: 250,
    customsFxSouth: 750,
    avgFxRate: 534
  ...
  // Get commodity data
  const commodityData = getCommodityDataByProductType(productTypeName);

  if (!commodityData) {
    console.warn(`⚠️ No commodity data found for: ${productTypeName}`);
    // Use default values
    const defaultData = {
      tariffSouth: 5,
      tariffNorth: 5,
    ...
    return calculateCustomsWithData(cifUsd, destination, exchangeRates, defaultData);
  }

  return calculateCustomsWithData(cifUsd, destination, exchangeRates, commodityData);
}

// Calculate customs with provided data
function calculateCustomsWithData(cifUsd, destination, exchangeRates, commodityData) {
  const isNorth = isNorthernProvince(destination);

  // Convert percentages to decimals
  const tariffRate = isNorth ? (commodityData.tariffNorth / 100) : (commodityData.tariffSouth / 100);
  const salesTaxRate = commodityData.salesTax / 100;
  const vatRate = commodityData.vat / 100;
  const profitTaxRate = commodityData.profitTax / 100;

  // ===================== الجنوب =====================
  const southCustomsValue = cifUsd * exchangeRates.customsFxSouth;
  const southTariff = southCustomsValue * (commodityData.tariffSouth / 100);
  ...
  const southPayableUSD = southPayableYER / exchangeRates.southMarketFx;

  // ===================== الشمال =====================
  let northPayableUSD = 0;
  let northCustomsValue = 0;
  let northTariffHalf = 0;
  let northMarketValue = 0;
  let northFullTariffForTax = 0;
  let northTaxable = 0;
  let northSalesTax = 0;
  let northVat = 0;
  let northProfitTax = 0;
  let northFixedFees = 0;
  let northPayableYER = 0;

  if (isNorth) {
    northCustomsValue = cifUsd * exchangeRates.customsFxNorth;
    northTariffHalf = northCustomsValue * (commodityData.tariffNorth / 100 / 2); // نصف التعرفة
    northMarketValue = cifUsd * exchangeRates.avgFxRate;
    northFullTariffForTax = northCustomsValue * (commodityData.tariffNorth / 100); // التعرفة الكاملة للضرائب
    northTaxable = northMarketValue + northFullTariffForTax;
    northSalesTax = northTaxable * salesTaxRate;
    northVat = northTaxable * vatRate;
    northProfitTax = northTaxable * profitTaxRate;
    northFixedFees = (northCustomsValue * 0.06) + 3000;

    northPayableYER = northTariffHalf + northSalesTax + northVat + northProfitTax + northFixedFees;
    northPayableUSD = northPayableYER / exchangeRates.avgFxRate;
  }

  const finalPayableUSD = isNorth ? (southPayableUSD + northPayableUSD) : southPayableUSD;
  ...
  return {
    ...
    commodityData,
    south: {
      customsValue: southCustomsValue,
      tariff: southTariff,
      taxable: southTaxable,
      salesTax: southSalesTax,
      vat: southVat,
      profitTax: southProfitTax,
      fixedFees: southFixedFees,
      payableYER: southPayableYER,
      payableUSD: southPayableUSD
    },
    north: isNorth ? {
      customsValue: northCustomsValue,
      tariffHalf: northTariffHalf,
      marketValue: northMarketValue,
      fullTariffForTax: northFullTariffForTax,
      taxable: northTaxable,
      salesTax: northSalesTax,
      vat: northVat,
      profitTax: northProfitTax,
      fixedFees: northFixedFees,
      payableYER: northPayableYER,
      payableUSD: northPayableUSD
    } : null,
    finalPayableUSD
  };
}

// Initialize customs data on page load
document.addEventListener('DOMContentLoaded', async () => {
  console.log('🔄 Initializing customs calculation system...');

  // Fetch exchange rates and commodity data
  await Promise.all([
    fetchExchangeRatesFromMonday(),
    fetchCommodityTypesFromMonday()
  ]);

  console.log('✅ Customs calculation system initialized');
});

/* ===== CUSTOMS RESULTS DISPLAY FUNCTION ===== */

// Display customs calculation results in HTML format
function displayCustomsResults(customsResult) {
  if (!customsResult) {
    return '';
  }

  const { isNorth, south, north, finalPayableUSD, exchangeRates, commodityData } = customsResult;

  // Build north breakdown if applicable
  let breakdownNorth = "";
  if (isNorth && north) {
    breakdownNorth = `
      <tr style="background: #eef2ff; font-weight: bold;"><td colspan="2">تحليل تحصيل الشمال</td></tr>
      <tr style="background: #fef3c7;"><td>التعرفة (نصف)</td><td>${north.tariffHalf.toLocaleString('ar-YE')} ريال</td></tr>
      <tr style="background: #dbeafe;"><td>ضريبة المبيعات</td><td>${north.salesTax.toLocaleString('ar-YE')} ريال</td></tr>
      <tr style="background: #dcfce7;"><td>ضريبة القيمة المضافة</td><td>${north.vat.toLocaleString('ar-YE')} ريال</td></tr>
      <tr style="background: #ede9fe;"><td>ضريبة الأرباح</td><td>${north.profitTax.toLocaleString('ar-YE')} ريال</td></tr>
      <tr style="background: #fee2e2;"><td>الصناديق</td><td>${north.fixedFees.toLocaleString('ar-YE')} ريال</td></tr>
      <tr><td><strong>إجمالي الشمال (USD)</strong></td><td><strong>${north.payableUSD.toFixed(2)} USD</strong></td></tr>
    `;
  }

  // Main customs breakdown table
  const customsBreakdownHtml = `
    <div style="margin-top: 24px; padding: 16px; background: #f9fafb; border-radius: 12px; border: 2px solid #e5e7eb;">
      <h3 style="margin: 0 0 16px 0; color: #0b1220; font-size: 18px; font-weight: 900;">
        📊 تفاصيل الجمارك والرسوم
      </h3>
      <table style="width: 100%; border-collapse: collapse; background: #fff; border-radius: 8px; overflow: hidden;">
        <tr style="background: #0b1220; color: #fff;">
          <th style="padding: 12px; text-align: right; font-size: 14px;">البند</th>
          <th style="padding: 12px; text-align: right; font-size: 14px;">القيمة</th>
        </tr>

        <tr style="background: #eef2ff; font-weight: bold;">
          <td colspan="2" style="padding: 10px;">تحليل تحصيل الجنوب</td>
        </tr>
        <tr style="background: #fef3c7;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">التعرفة</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.tariff.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr style="background: #dbeafe;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">ضريبة المبيعات</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.salesTax.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr style="background: #dcfce7;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">ضريبة القيمة المضافة</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.vat.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr style="background: #ede9fe;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">ضريبة الأرباح</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.profitTax.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr style="background: #fee2e2;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">الصناديق</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.fixedFees.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>إجمالي الجنوب (USD)</strong></td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>${south.payableUSD.toFixed(2)} USD</strong></td>
        </tr>

        ${breakdownNorth}

        <tr style="background: #111827; color: #fff; font-size: 16px; font-weight: bold;">
          <td style="padding: 14px;">✅ المبلغ النهائي المستحق (الجمارك)</td>
          <td style="padding: 14px;">${finalPayableUSD.toFixed(2)} USD</td>
        </tr>
      </table>
    </div>
  `;

  // Base values reference table
  const baseValuesHtml = `
    <div style="margin-top: 16px; padding: 16px; background: #fff7ed; border-radius: 12px; border: 2px solid #fed7aa;">
      <h3 style="margin: 0 0 16px 0; color: #0b1220; font-size: 18px; font-weight: 900;">
        📋 القيم المرجعية للحساب
      </h3>
      <table style="width: 100%; border-collapse: collapse; background: #fff; border-radius: 8px; overflow: hidden;">
        <tr style="background: #eef2ff; font-weight: bold;">
          <td colspan="2" style="padding: 10px;">الجنوب</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة الجمركية – جنوب</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.customsValue.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة السوقية – جنوب</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.customsValue.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة الخاضعة للضريبة – جنوب</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${south.taxable.toLocaleString('ar-YE')} ريال</td>
        </tr>

        ${isNorth && north ? `
        <tr style="background: #eef2ff; font-weight: bold;">
          <td colspan="2" style="padding: 10px;">الشمال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة الجمركية – شمال</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${north.customsValue.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة السوقية – شمال</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${north.marketValue.toLocaleString('ar-YE')} ريال</td>
        </tr>
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">القيمة الخاضعة للضريبة – شمال</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${north.taxable.toLocaleString('ar-YE')} ريال</td>
        </tr>
        ` : ''}

        <tr style="background: #f3f4f6;">
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>التعرفة المستخدمة</strong></td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>${commodityData.tariffSouth}% (جنوب)${isNorth ? ` / ${commodityData.tariffNorth}% (شمال)` : ''}</strong></td>
        </tr>
      </table>

      <div style="margin-top: 12px; padding: 10px; background: #fff; border-radius: 8px; border-right: 3px solid #f59e0b;">
        <div style="font-size: 12px; color: #92400e; line-height: 1.6;">
          <strong>📌 ملاحظة:</strong> ${isNorth ? 'الشحنات للشمال تمر بجمارك الجنوب أولاً ثم الشمال. التعرفة في الشمال تُحسب بنصف القيمة فقط.' : 'الشحنات للجنوب تخضع لجمارك الجنوب فقط.'}
        </div>
      </div>
    </div>
  `;

  return customsBreakdownHtml + baseValuesHtml;
}

// Helper function to add customs to results display
function addCustomsToResults(itemValue, productType, destination, resultsContainer) {
  if (!itemValue || !productType || !destination || !resultsContainer) {
    return;
  }

  try {
    // Calculate customs
    const customsResult = calculateCustomsDuties(itemValue, productType, destination);

    // Generate HTML
    const customsHtml = displayCustomsResults(customsResult);

    // Append to results container
    const customsDiv = document.createElement('div');
    customsDiv.id = 'customsResultsSection';
    customsDiv.innerHTML = customsHtml;
    resultsContainer.appendChild(customsDiv);

    console.log('✅ Customs results added to display');
    return customsResult;
  } catch (error) {
    console.error('❌ Error displaying customs results:', error);
    return null;
  }
}
